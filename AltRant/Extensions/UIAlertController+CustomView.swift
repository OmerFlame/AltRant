...
//  AltRant
//
//  Created by Omer Shamai on 23/05/2021.
//
...
extension UIAlertController {
	...
	convenience init(title: String?, customView: UIView, fallbackMessage: String?, preferredStyle: UIAlertController.Style) {
		...
		if let customContentPlaceholder = self.view.findLabel(withText: marker),
			...
			customContentPlaceholder.tintColor = .red
			
			view.findLabel(withText: "You need 8000 ++'s in order to equip this.")!.tintColor = .red
			view.findLabel(withText: "You need 8000 ++'s in order to equip this.")!.textColor = .red
			view.findLabel(withText: "You need 8000 ++'s in order to equip this.")!.layer.sublayers?.removeAll()
			
			view.tintColor = .red
			//customContainer.tintColor = .black
